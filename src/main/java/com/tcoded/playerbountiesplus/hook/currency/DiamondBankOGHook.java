...
public class DiamondBankOGHook implements EconomyHook {
        ...
        try {
            ...
            diamondBank.addToPlayerBankShards(
                    player.getUniqueId(), shards, "Player " + player.getName() + " defeated " + victim.getName()
                            + " and earned a bounty of " + diamondBank.shardsToDiamonds(shards) + ".",
                    "Plugin: PlayerBounties-OG");

        } catch (EconomyDisabledException economyDisabledError) {

            PlayerBountiesOG
                    .disableSelf("[PlayerBounties-OG] Paying " + player.getName() + " for killing " + victim.getName()
                            + " failed because the economy is disabled! " + economyDisabledError.getMessage());
    ...
    public boolean takeEco(Player player, Player victim, double diamonds) {
        ...
        try {
            ...
            diamondBank
                    .consumeFromPlayer(player.getUniqueId(), shards,
            ...
            PlayerBountiesOG.disableSelf(
                    "[PlayerBounties-OG] DiamondBank-OG economy disabled: " + economyDisabledError.getMessage());
            economyDisabledError.printStackTrace();
            ...
            UtilitiesOG.trueogMessage(player,
                    "&6WARNING: You do not have enough Diamonds to contribute to that bounty.");
